...
struct lista {
	...
	// Para todas as funções relacionadas as "...Fim", cabeça serve como um ponteiro temporário,
	// já nos outros tem que ser a original.
	
	// Caso esteja começando uma linked list, a cabeça deve ser igual a "nullptr/0"
	void adicionarNodo(lista** cabeça, T dado)
	{
...
#pragma warning( disable : 6011 )
		tempNodo->dado = dado;
#pragma warning( pop )
		tempNodo->prox_nodo = *cabeça;

		*cabeça = tempNodo;
	}
	
	void adicionarNodo(lista** cabeça, lista* nodo)
	{
		nodo->prox_nodo = *cabeça;
		*cabeça = nodo;
	}

	void adicionarFim(lista* cabeça, T dado)
	{
		for (; cabeça->prox_nodo;) { cabeça = cabeça->prox_nodo; }

		// Não vou testar se é null

		cabeça->prox_nodo = (lista*)VirtualAlloc(0, sizeof lista, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
#pragma warning( push )
#pragma warning( disable : 6011 )
		cabeça->prox_nodo->dado = dado;
#pragma warning( pop )
	}
	
	void adicionarFim(lista* cabeça, lista* nodo)
	{
		for (; cabeça->prox_nodo;) { cabeça = cabeça->prox_nodo; }

		cabeça->prox_nodo = nodo;
	}
	...
	void excFim(lista* cabeça)
	{
		for (; cabeça->prox_nodo->prox_nodo;) { cabeça = cabeça->prox_nodo; }

		VirtualFree(cabeça->prox_nodo, 0, MEM_RELEASE);

		cabeça->prox_nodo = nullptr;
	}

	void excComeco(lista** cabeça)
	{
		lista* tempPtr = (*cabeça)->prox_nodo;

		VirtualFree(*cabeça, 0, MEM_RELEASE);
